...
OUTDIR = Path("eval/cond_v1_novel_250")
DATA = "data/small_d4_props_anon_src.csv"
CKPT_DIR = Path("/data/rkumar/code/py/ed/checkpoints/sweep_novelty/v1_anon_src")
N = 250
SEED = 42
...
def step5_ehull_filter(bvs_passed):
    """Filter by ehull using 4 parallel GPUs."""
    print(f"\n=== Step 5: Ehull ({len(bvs_passed)} structures, 4 GPUs) ===", flush=True)

    def ehull_worker(gpu_id, atoms_chunk, q):
        ...
        for i, atoms in enumerate(atoms_chunk):
            try:
                ...
                results.append((atoms, r["ehull_pass"]))
            except Exception:
                atoms.info["ehull_pass"] = False
                results.append((atoms, False))
        q.put(results)

    chunk_size = math.ceil(len(bvs_passed) / 4)
    chunks = [bvs_passed[i:i + chunk_size] for i in range(0, len(bvs_passed), chunk_size)]

    t0 = time.time()
    q = Queue()
    procs = []
    for gpu_id, chunk in enumerate(chunks):
        p = Process(target=ehull_worker, args=(gpu_id, chunk, q))
        p.start()
        procs.append(p)
        print(f"  GPU {gpu_id}: {len(chunk)} structures", flush=True)
