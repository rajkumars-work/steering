...
The crystal mirror of the ImageNet experiment
(~/code/py/imagenet_steering_verify/src/claim1_survives.py).

  target : DENSITY (rho, g/cm^3)
  bins   : chemistry  =  (element set, atom count)

We read the within/between split off the DATA, then GENERATE from CUES, bin the model's
own outputs the same way, and recompute the split. If the data-side budget is real, the
model reproduces it.

WHY DENSITY (not e_above_hull).
A first run used e_above_hull and it did NOT survive — but that test is confounded: stability
is both (i) the property the training set was *curated* on (so the data spread is artificially
tiny) and (ii) the property models are worst at. Density is the fair, brightness-like analog:
every generated structure has one, the model produces it reliably, it is NOT a curation
criterion, and it is pure geometry (mass / cell volume) — so no MACE, no hull, no inf, and the
run takes minutes, not hours.

ONE MEASURE FOR BOTH SIDES.
Density is computed the same way for data structures and generated structures (g/cm^3), so the
two are on one scale — exactly as the image run used one brightness function for both.

WITHIN-BIN SPREAD NEEDS MULTI-MEMBER BINS.
Most (elements, atom-count) bins are near-singletons; pick bins with >= MIN_MEMBERS real
structures so the data within-bin variance is meaningful. The model always gives a spread.

WHAT TO EXPECT (then let the numbers decide).
Does the model reproduce the data's density budget — the dominant between-chemistry part
(chemistry largely sets density) and the within-chemistry spread? Report total / within /
between for both sides, the ratios, and the per-chemistry mean correlation.

OUTPUT: prints a data-vs-model table and writes JSON to OUT. Fast (no scorer to load).
"""
...
DEVICE    = "cuda"
TRAIN_CSV = "/opt/dlami/nvme/recast/train/alex_nolemat_lowhull_dataset_train.csv"
OUT       = "/opt/dlami/nvme/recast/train/claim1_survival_crystals_density.json"
...
def main():
    ...
    log(f"wrote {OUT}")
    print("CLAIM1-CRYSTAL-SURVIVAL-DONE")
