...
CONVERGED on the property. We report it two ways:

  (A) CONVERGENCE LADDER — pass/quality rate, data vs model, for SMACT -> BVS -> validity ->
      (meta)stable. Expect the model's rate to fall as the check gets harder.
  (B) BUDGET SURVIVAL — within (T) / between (E) split + per-chemistry mean r^2, for the
      CONTINUOUS properties density -> BVS GII -> e_above_hull. Expect r^2 / E-match to
      fall from density to stability.

check_validity() runs SMACT + BVS + min-dist in one call (and returns bvs_max_deviation), so
one validity call + geometry + one MACE call covers everything per structure. One measure per
property for BOTH data and generated structures.

Cost: MACE loads once, then ~K*(M_data+M_gen) scorings — multi-hour, like binning_experiment.py.
Output: a ladder table + a survival table, and JSON to OUT.
"""
...
CKPT      = "/opt/dlami/nvme/recast/train/mix_ep120_ckpt"
VERSION   = "d15_binrho_k7"
DEVICE    = "cuda"
TRAIN_CSV = "/opt/dlami/nvme/recast/train/alex_nolemat_lowhull_dataset_train.csv"
OUT       = "/opt/dlami/nvme/recast/train/claim1_survival_spectrum.json"
...
def bvs_gii(a):
    ...
    be assigned (e.g. pure intermetallics with no anion) or the structure is unusable — those
    simply don't contribute to the BVS rung. (NOTE: this replaces check_validity's
    `bvs_max_deviation`, which is |valence - round(valence)| on INTEGER assigned valences and so
    is structurally ~0 — useless as a continuous signal.)"""
    try:
        from pymatgen.io.ase import AseAtomsAdaptor
